class UploadVideoJob < ApplicationJob
  queue_as :default

  def perform(video_id = nil)
    return unless video_id
    
    video = Video.find_by(id: video_id)
    return unless video
    
    update_progress(0, "Starting video upload")
    
    begin
      update_progress(25, "Preparing video metadata")
      update_progress(50, "Uploading to YouTube")
      update_progress(75, "Processing on YouTube")
      YouTubeUploader.new.upload(video)
      update_progress(100, "Completed video upload")
    rescue StandardError => e
      update_progress(-1, "Error: #{e.message}")
      Rails.logger.error "Failed to upload video #{video_id}: #{e.message}"
      raise
    end
  end

  private

  def update_progress(percentage, message)
    Sidekiq.redis do |conn|
      conn.hset(
        "job_progress:#{self.class.name}:#{job_id}",
        {
          percentage: percentage,
          message: message,
          updated_at: Time.current.to_i
        }
      )
      # Set expiry to 1 hour
      conn.expire("job_progress:#{self.class.name}:#{job_id}", 3600)
    end
  end
end 